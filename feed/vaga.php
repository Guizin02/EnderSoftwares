...
<body>
    ...
    <div id="feed">
        <div class="foryou">
    <?php $codvaga = $_GET['cod']; 
    @session_start();
	$_SESSION['userLogin'] = $user_id;
    require('connect.php');
    $vagas = mysqli_query($con, "SELECT * FROM `vagas` INNER JOIN `user` ON user.id_cliente = vagas.id_usuario WHERE `id_vaga` = '$codvaga'");
    $vaga = mysqli_fetch_array($vagas);
    echo "<div class=publicacaofeed>";
        echo "<div class=head>";
            ...
            <?php
                   ...
                   echo "<p>R$ $vaga[orcamento_vaga]</p>";
                   echo "</div>";
                    echo "<div class='caption vaga'>";
                        echo "<h2><strong>$vaga[titulo_vaga]</strong> </h2>";
                        echo "<p>$vaga[descricao_vaga]</p>";
                    echo "</div>";
                    echo "<div class='feed-phots' id='imgVaga'>";
                        echo "<img src=imgsvagas/$vaga[imagem_vaga] alt='Imagem da Publicação' id=imgPublicacao>";
                        echo "<img src=imgsvagas/$vaga[imagem2_vaga] alt='Imagem da Publicação' id=imgPublicacao>";
                    echo "</div>";
                        echo "<div class='feed-phots' id='imgVaga'>";
                        echo "<img src=imgsvagas/$vaga[imagem3_vaga] alt='Imagem da Publicação' id=imgPublicacao>";
                        echo "<img src=imgsvagas/$vaga[imagem4_vaga] alt='Imagem da Publicação' id=imgPublicacao>";
                    echo "</div>";
