...
<body id="feedbody">
  ...
  <div id="feed">
    ...
    require('connect.php');
    $publicacoes = mysqli_query($con, "SELECT * FROM `publicacoes` INNER JOIN user ON user.id_cliente = publicacoes.codUsuario WHERE codPublicacao = $codPubli ORDER BY codPublicacao DESC");
  ...
  echo "<p class='curtir' data-publi=$publicacao[codPublicacao]> $publicacao[curtidas] curtidas</p>";
    ...
    <form action=comentario.act.php method=post enctype=multipart/form-data id='form-creat-comment'>
        <div class="usuario">
            ...
            <div class="txtcomentario">
                ...
                <?php
    echo "</div>";
    $comentario = mysqli_query($con, "Select * from `comentarios` INNER JOIN user ON user.id_cliente = comentarios.codUsuario WHERE `codPublicacao` = $publicacao[codPublicacao] ORDER BY codComentario DESC");
    ...
    echo "<div class=box-comment>";
        echo "<div class=info>";
            ...
            echo "</div>";
                echo "<h3 class=comment-username><strong>$comentarios[nome]</strong></h3>";
        echo "</div>";
        echo "<div class=comment-user>";
            echo "<p id=comentarioUsuario>$comentarios[comentario]</p>";
        echo "</div>";
        echo "<small>$comentarios[dataComentario]</small>";
    echo "</div>";
    }
    }
    ?>
